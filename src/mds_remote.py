...
class RemoteEnv(BaseEnv):
    ...
    def __init__(self, remote):
        ...
        if env0[0] == 0 and not env0[2].rstrip():
            ...
            runs = (
                (key, session.run('printenv "%s"; echo' % key)) for key in keys
            )
            self._curr = dict(
                (key, run[1].rstrip("\n"))
                for (key, run) in runs
    ...
    * ``connect_timeout`` - the connection timeout
    ...
    def _path_glob(self, fn, pattern):
        ...
        matches = self._session.run(
            r"for fn in {0}/{1}; do echo $fn; done".format(fn, pattern)
        )[1].splitlines()
    ...
    def _path_stat(self, fn):
        ...
        res = StatRes(
            (int(statres[0], 16),) + tuple(int(sr) for sr in statres[1:])
        )
    ...
    def expanduser(self, expr):
        ...
        return self._session.run("echo %s" % (expr.replace("$", "\\$"),))[
            1
        ].strip()
